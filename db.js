  ...
  db.serialize(() => {
    ...
    const tables = [
      ...
      },
      {
        ...
        sql: `CREATE TABLE IF NOT EXISTS kredi_kurallari (id INTEGER PRIMARY KEY AUTOINCREMENT, kural_adi TEXT NOT NULL, kredi INTEGER NOT NULL CHECK(kredi >= 0), tarih TEXT, sabit_kural INTEGER DEFAULT 0 CHECK(sabit_kural IN (0, 1)), created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(kural_adi, tarih))`
      },
      {
        name: 'nobet_kredileri',
        sql: `CREATE TABLE IF NOT EXISTS nobet_kredileri (id INTEGER PRIMARY KEY AUTOINCREMENT, kredi_dakika INTEGER NOT NULL CHECK(kredi_dakika > 0), baslangic_saat TEXT NOT NULL, bitis_saat TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(baslangic_saat, bitis_saat))`
      },
      {
        name: 'users',
        sql: `CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL UNIQUE COLLATE NOCASE, password TEXT NOT NULL, role TEXT DEFAULT 'user' CHECK(role IN ('user', 'admin')), email TEXT UNIQUE, reset_password_token TEXT, reset_password_expires INTEGER, last_login DATETIME, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`
      },
      {
        name: 'takvim_aciklamalari',
        sql: `CREATE TABLE IF NOT EXISTS takvim_aciklamalari (id INTEGER PRIMARY KEY AUTOINCREMENT, yil INTEGER NOT NULL CHECK(yil >= 2020 AND yil <= 2050), hafta INTEGER NOT NULL CHECK(hafta >= 1 AND hafta <= 53), aciklama TEXT, nobetci_id_override INTEGER DEFAULT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, UNIQUE(yil, hafta), FOREIGN KEY (nobetci_id_override) REFERENCES Nobetciler(id) ON DELETE SET NULL)`
      },
      {
        name: 'uygulama_ayarlari',
        sql: `CREATE TABLE IF NOT EXISTS uygulama_ayarlari (ayar_key TEXT PRIMARY KEY, ayar_value TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`
      }
    ];
    tables.forEach(table => db.run(table.sql, (err) => { if (err) logger.error(`Error creating table ${table.name}:`, err); }));
    const indexes = ['CREATE INDEX IF NOT EXISTS idx_nobetciler_is_aktif ON Nobetciler(is_aktif)', 'CREATE INDEX IF NOT EXISTS idx_nobetciler_telegram_id ON Nobetciler(telegram_id)', 'CREATE INDEX IF NOT EXISTS idx_kredi_kurallari_tarih ON kredi_kurallari(tarih)', 'CREATE INDEX IF NOT EXISTS idx_takvim_yil_hafta ON takvim_aciklamalari(yil, hafta)', 'CREATE INDEX IF NOT EXISTS idx_users_username ON users(username)', 'CREATE INDEX IF NOT EXISTS idx_users_email ON users(email)'];
    indexes.forEach(indexSql => db.run(indexSql, (err) => { if (err) logger.error('Error creating index:', err); }));
